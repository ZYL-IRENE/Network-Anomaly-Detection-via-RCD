...
pictureWay = 'Pics/result_kddcup10percent.png'
# 读入数据
source = 'Data/normal_kddcup10percent.csv'
instances = []
labels = []
count = 0
...
with open(source, 'rb') as sourcefp:
    for line in sourcefp:
        ...
        coordinate = (float(tmp[0]), int(float(tmp[1])), int(float(tmp[2])), int(float(tmp[3])), float(tmp[4]),
                      float(tmp[5]), int(float(tmp[6])), float(tmp[7]), float(tmp[8]), float(tmp[9]), float(tmp[10]))
